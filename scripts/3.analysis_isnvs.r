...
files_bam_rst_full <- list.files("../results/vcf/", "csv$", full.names = T)
df_snvs <- read_snpeff(files_bam_rst_full)
...
df_bam_rst <- read_pysamstats(files_bam_rst_full)
...
df_indels_meta_add_qc <- df_indels_meta_add_qc %>% filter(sec_freq>=0.03) 
...
wilcox.test(df_plot_n$n_per_kb[df_plot_n$lineage_sim=="Delta" & df_plot_n$Vaccine=="Non-vaccinated"], df_plot_n$n_per_kb[df_plot_n$lineage_sim=="Delta" & df_plot_n$Vaccine!="Non-vaccinated"])

wilcox.test(df_plot_n$n_per_kb[df_plot_n$Vaccine=="Sinovac"], df_plot_n$n_per_kb[df_plot_n$Vaccine=="BioNTech"])
wilcox.test(df_plot_n$n_per_kb[df_plot_n$Vaccine=="Non-vaccinated"], df_plot_n$n_per_kb[df_plot_n$Vaccine=="BioNTech"])
wilcox.test(df_plot_n$n_per_kb[df_plot_n$Vaccine=="Non-vaccinated"], df_plot_n$n_per_kb[df_plot_n$Vaccine=="Sinovac"])
...
wilcox.test(df_plot_n$n_per_kb[df_plot_n$`Vaccine type`=="Non-vaccinated"], df_plot_n$n_per_kb[df_plot_n$`Vaccine type`=="Adeno"])
wilcox.test(df_plot_n$n_per_kb[df_plot_n$Vaccine=="Non-vaccinated"], df_plot_n$n_per_kb[df_plot_n$Vaccine!="Non-vaccinated"])

ggplot(df_plot, aes(x=alle_freq))+ # freq of isnvs
	geom_histogram(binwidth = 0.01)+
	scale_x_continuous(breaks = 1:20/20)+
	facet_wrap(vars(Vaccine), scales="free_x", ncol=1)

ggplot(df_plot, aes(x=sample, y=alle_freq))+ # freq of isnvs
	geom_boxplot()+
	facet_wrap(vars(Vaccine), scales="free_x", ncol=1)

ggplot(df_plot, aes(x=sample, y=alle_freq, color=effect))+ # freq of isnvs
	geom_boxplot()+
	facet_wrap(vars(Vaccine), scales="free_x", ncol=1)

ggplot(df_plot, aes(x=Vaccine, y=alle_freq))+ # freq of isnvs
	geom_boxplot()+
	# facet_wrap(vars(Vaccine), scales="free", ncol=1)+
	NULL

wilcox.test(df_plot$alle_freq[df_plot$Vaccine=="Non-vaccinated"], df_plot$alle_freq[df_plot$Vaccine=="BioNTech"])
wilcox.test(df_plot$alle_freq[df_plot$Vaccine=="Non-vaccinated"], df_plot$alle_freq[df_plot$Vaccine=="Sinovac"])
wilcox.test(df_plot$alle_freq[df_plot$Vaccine=="Non-vaccinated"], df_plot$alle_freq[df_plot$Vaccine!="Non-vaccinated"])
